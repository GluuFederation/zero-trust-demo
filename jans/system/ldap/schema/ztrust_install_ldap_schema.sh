...
echo "Copying file: ztrust-jans-attributes.ldif";
cp ztrust-jans-attributes.ldif "$opendj_dpath/ldif";
if [ $? == 0 ]; then
    echo "File: ztrust-jans-attributes.ldif has been copied";
else
    echo "File: ztrust-jans-attributes.ldif hasn't been copied";
    exit 1;
fi;

echo "Setup owner 'ldap:ldap' of the file: $opendj_dpath/ldif/ztrust-jans-attributes.ldif";
chown ldap:ldap "$opendj_dpath/ldif/ztrust-jans-attributes.ldif";
if [ $? == 0 ]; then
    echo "Owner 'ldap:ldap' is Ok";
else
    echo "Owner 'ldap:ldap' isn't Ok";
...
$opendj_dpath/bin/ldapmodify -h localhost -p 1636 -Z -D "cn=Directory Manager" -j "$pw_fpath" -a -f "$opendj_dpath/ldif/ztrust-jans-attributes.ldif";
if [ $? == 0 ]; then
